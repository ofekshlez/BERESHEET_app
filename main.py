...
def tasks(id_num):
    ...
    if collection2.count_documents({"school": school}, limit=1) == 0:
        return "0\n"
    ...
    if not assigned:
        return "0\n"
    for i in assigned:
        left += rev_dic_of_quizzes.get(i[0]) + "," + str(i[1]) + ","
...
if __name__ == '__main__':
    while True:
        ...
        elif info.get("function") == "tasks":
            client_socket.sendall(tasks(info.get("id")).encode())
        client_socket.close()
        server_socket.close()